...
function setupDownloadBtn() {
  if (
    ...
    document.getElementById("downloadBtnContainer") === null
  ) {
    try {
...
/**
 * Removes YouTubes own download-button from the page.
 */
function removeExistingDownloadBtn() {
  let downloadBtn = document.getElementById("top-level-buttons-computed")
    .childNodes[3];
...
function createDownloadBtn() {
  ...
  document
    .getElementsByClassName(
      "top-level-buttons style-scope ytd-menu-renderer"
    )[0]
...
function getFilenameForDownload() {
  let videoTitle = document.getElementsByClassName(
    "title style-scope ytd-video-primary-info-renderer"
  )[0].childNodes[0].innerHTML;
  chrome.runtime.sendMessage({
    filename: videoTitle.replace(/["&;|]/g, ""),
  });
}
